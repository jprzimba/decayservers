...
bool Chat::parseChannelNode(xmlNodePtr p)
{
	...
	if(readXMLInteger(p, "muted", intValue))
	{
		...
		if((condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_MUTED, tmp, conditionId)))
		{
...
bool Chat::talkToChannel(Player* player, SpeakClasses type, const std::string& text, uint16_t channelId)
{
	...
	if(text.substr(1) == "disband")
	{
		if(player->getGuildLevel() == GUILDLEVEL_LEADER)
		{
			...
			channel->talk(player, SPEAK_CHANNEL_RA, "The guild has been disbanded.");
				...
				if(g_game.getPlayerByNameWildcard(param, paramPlayer) == RET_NOERROR)
				{
					if(paramPlayer->getGuildId() == 0)
					{
						if(!paramPlayer->isGuildInvited(player->getGuildId()))
						{
							...
							channel->talk(player, SPEAK_CHANNEL_RA, buffer);
	...
	else if(text.substr(1, 9) == "cleanmotd")
	{
		...
		player->sendToChannel(player, SPEAK_CHANNEL_RA, "Guild commands with parameters: disband, invite[name], leave, kick[name], revoke[name], demote[name], promote[name], passleadership[name], nick[name, nick], setrankname[oldName, newName], setmotd[text] and cleanmotd.", CHANNEL_GUILD);
	else
		return false;
