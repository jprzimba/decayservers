...
elseif($page == 'menu')
{
	...
	<b>Modified:</b><br>
	Tryller<br>
	Compatible with TFS 0.2 (7.92)</a>';
}
elseif($page == 'step')
{
	if($step >= 2 && $step <= 5)
	...
	elseif($step == 4)
	{
		...
		$newPlayer = new Player('Account Manager', Player::LOADTYPE_ID);
		if($newPlayer->isLoaded())
		{
			foreach($samplePlayers as $vocationID => $name)
			{
				$samplePlayer = new Player($name, Player::LOADTYPE_ID);
				if(!$samplePlayer->isLoaded())
				{
					$samplePlayer = new Player('Account Manager', Player::LOADTYPE_ID);
