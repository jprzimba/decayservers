...
if(!$logged)
    ...
    if($action == '')
    {
        ...
        foreach($sections as $id => $section)
        {
            ...
            if(isset($counters[$id]['posts']))
                $main_content .= (int) $counters[$id]['posts'];
            else
                $main_content .= '0'; // Defina um valor padrão caso o índice 'posts' não exista no array $counters[$id]
            
            $main_content .= '</td><td>';
            
            if(isset($counters[$id]['threads']))
                $main_content .= (int) $counters[$id]['threads'];
            else
                $main_content .= '0'; // Defina um valor padrão caso o índice 'threads' não exista no array $counters[$id]
...
if($action == 'show_board')
{
    ...
    if(isset($last_threads[0]))
    {
        ...
        foreach($last_threads as $thread)
        {
            ...
            if($logged && $group_account >= $group_not_blocked)
...
if($action == 'show_thread')
{
    ...
    if(!empty($thread_name['name']))
    {
        ...
        foreach($threads as $thread)
        {
            ...
            if($logged && ($thread['account_id'] == $account_logged->getId() || $group_account >= $group_not_blocked))
...
if($action == 'new_post')
{
    if($logged)
    {
        if(canPost($account_logged) || $group_account >= $group_not_blocked)
        {
            ...
            if(isset($thread['id']))
            {
                ...
                elseif(isset($_REQUEST['save']))
                {
                    ...
                    if(count($errors) == 0)
                    {
                        ...
                        if($last_post+$post_interval-time() > 0 && $group_account < $group_not_blocked)
    ...
    if($logged)
    {
        if(canPost($account_logged) || $group_account >= $group_not_blocked)
        {
            ...
            if(isset($thread['id']))
            {
                ...
                if($account_logged->getId() == $thread['author_aid'] || $group_account >= $group_not_blocked)
                {
                    $players_from_account = $SQL->query("SELECT " . $SQL->tableName('players') . "." . $SQL->fieldName('name') . ", " . $SQL->tableName('players') . "." . $SQL->fieldName('id') . " FROM " . $SQL->tableName('players') . " WHERE " . $SQL->tableName('players') . "." . $SQL->fieldName('account_id') . " = ".(int) $account_logged->getId())->fetchAll();
...
if($action == 'new_topic')
{
    ...
    if($logged)
    {
        if(canPost($account_logged) || $group_account >= $group_not_blocked)
        {
            ...
            {
                if($section_id == 1 && $group_account < $group_not_blocked)
