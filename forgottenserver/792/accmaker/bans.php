...
<?php
$SQL = AAC::$SQL;
$SQL->myQuery('SELECT players.name, bans.time FROM bans, players WHERE bans.type = 1 AND players.id = bans.account OR bans.type = 2 AND players.id = bans.account OR bans.type = 3 AND players.account_id = bans.account ORDER BY time ASC');
